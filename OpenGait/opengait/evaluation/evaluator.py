...
# Cho phép dùng cả 'embeddings' lẫn 'features'
def _get_features(data_dict):
    if 'embeddings' in data_dict:
        return data_dict['embeddings']
    if 'features' in data_dict:        # một số loader dùng tên này
        return data_dict['features']
    raise KeyError("Thiếu 'embeddings' (hoặc 'features') trong data.")
...
def cross_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    probe_seq_dict = {'CASIA-B': {'NM': ['nm-01'], 'BG': ['bg-01'], 'CL': ['cl-01']},
                      'OUMVLP': {'NM': ['00']}}
    gallery_seq_dict = {'CASIA-B': ['nm-02',],
                        'OUMVLP': ['01']}
    msg_mgr = get_msg_mgr()
    acc = {}
    mean_ap = {}
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        ...
        for (v1, probe_view) in enumerate(view_list):
            ...
            # In kết quả từng mẫu test
            for i in range(len(probe_y)):
                min_dist_idx = np.argmin(dist[i].cpu().numpy())
                pred_label = gallery_y[min_dist_idx]
                correct = (probe_y[i] == pred_label)
                # msg_mgr.log_info(f"{type_} - View {probe_view} - Sample {i}: Probe label={probe_y[i]}, Predicted={pred_label}, Correct={correct}")

            eval_results = compute_ACC_mAP(dist.cpu().numpy(), probe_y, gallery_y, view[pseq_mask], view[gseq_mask])
            acc[type_][v1] = np.round(eval_results[0] * 100, 2)
            mean_ap[type_][v1] = np.round(eval_results[1] * 100, 2)

    result_dict = {}
    msg_mgr.log_info('===Cross View Gallery Evaluation (Excluded identical-view cases)===')
...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    probe_seq_dict = {'CASIA-B': {'NM': ['nm-01'], 'BG': ['bg-01','bg-02'], 'CL': ['cl-01', 'cl-02']},
                      'OUMVLP': {'NM': ['00']},
    ...
    target_view = ["000", "036","090", "144"]
    # target_view = ["090"]
    view_list = [target_view]
    num_rank = 1
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        ...
        pseq_mask = np.isin(seq_type, probe_seq) & np.isin(view, [target_view])
        probe_x = feature[pseq_mask, :]
        probe_y = label[pseq_mask]

        gseq_mask = np.isin(seq_type, gallery_seq_dict[dataset]) & np.isin(view, [target_view])
        gallery_y = label[gseq_mask]
        gallery_x = feature[gseq_mask, :]
        ...
        # Tính accuracy tổng
        acc[type_][0, 0, :] = np.round(
            np.sum(np.cumsum(np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 0:num_rank]], 1) > 0,
            0) * 100 / dist.shape[0], 2)

        # In kết quả từng mẫu test
        for i in range(len(probe_y)):
            correct = (probe_y[i] == gallery_y[idx[i, 0]])
            msg_mgr.log_info(f"{type_} - Sample {i}: Probe label = {probe_y[i]}, Predicted = {gallery_y[idx[i, 0]]}, Correct: {correct}")

    result_dict = {}
    # msg_mgr.log_info(f'=== Đánh giá với góc nhìn {target_view} ===')
    msg_mgr.log_info(f'=== Đánh giá với góc nhìn 090 ===')
    for type_ in probe_seq_dict[dataset].keys():
        rank1_acc = acc[type_][0, 0, 0]
        msg_mgr.log_info(f'{type_}@R1: {rank1_acc:.2f}%')
        result_dict[f'scalar/test_accuracy/{type_}@R1'] = rank1_acc
