...
class DeepGaitV2(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.summary_writer = SummaryWriter(log_dir='/content/drive/MyDrive/pbl4/OpenGait/logs')  # Đảm bảo đường dẫn đúng
    ...
    def forward(self, inputs):
        ...
        # In ra thông tin iteration và ghi video
        if torch.distributed.get_rank() == 0 and self.training and self.iteration % 100 == 0:
            print(f"Recording video at iteration {self.iteration}")  # In ra thông tin iteration
            print(f"out4 size: {out4.size()}")  # In ra kích thước của out4 để kiểm tra
            self.summary_writer.add_video('outs', out4.mean(2).unsqueeze(2), self.iteration)
